# Safety Review BI Prototype

## 📊 Overview
This project demonstrates a straightforward **Excel-to-Power BI workflow** designed to visualize safety performance metrics within a manufacturing environment.  
It highlights how simple data connections and dashboard design can support safety leaders in monitoring incidents, audits, and compliance trends.

The goal is to showcase proficiency in:
- Building **Power BI dashboards** from Excel data sources  
- Designing **clear, actionable visuals** for operational decision-making  
- Applying **data storytelling** to communicate safety insights effectively  

---

## 🧠 Project Architecture

### Data Flow
1. **Excel Source Files** → AI-generated sample safety data  
2. **Power BI Desktop** → Data modeling, visualization, and dashboard creation  

### Dashboard Structure
- **Business Safety Review Dashboard** – Executive overview of safety KPIs  
- **Incident Matrix & Severity Breakdown** – Visual representation of incident trends  
- **Audit & Compliance Tracking** – Monitoring inspection progress and completion rates  
- **Quarterly Metrics** – Year-over-year comparisons for safety performance  

---

## 🧩 Key Features
- **Excel Integration:** Imported structured safety data directly into Power BI.  
- **Data Modeling:** Created relationships between incident, audit, and compliance tables.  
- **DAX Measures:** Calculated KPIs such as incident frequency, audit completion rate, and severity index.  
- **Dashboard Design:** Developed an executive-style layout for safety leaders to review performance at a glance.  

---

## 📈 Dashboard Pages
1. **Executive Overview** – Summary of key safety KPIs  
2. **Incident Trends** – Rolling 12-month incident rate visualization  
3. **Audit & Compliance** – Tracking open and completed audits  
4. **Severity Breakdown** – Pie charts and matrices for incident severity  
5. **Quarterly Metrics** – Comparative analysis across years  

---

## 🧮 Tech Stack
| Tool | Purpose |
|------|----------|
| **Excel** | Source data for safety operations |
| **Power BI** | Data modeling, DAX, and visualization |
| **Copilot AI** | Assisted documentation and workflow optimization |

---

## 🧰 Setup Instructions
1. Place the Excel safety dataset into the `/Raw Files` directory.  
2. Open the Power BI file in `/Dashboard Images` or `/PowerBI Model`.  
3. Refresh data connections to load the latest Excel data.  
4. Explore visuals and KPIs across dashboard pages.

---

## 📂 Repository Structure


